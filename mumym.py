...
# 0.9.2:
#
#     + Refactor AI stuff to be a little more sane. (treed)   + Refactor AI stuff to be a little more sane. (treed)
#
# 0.9.1:
#
...
class Mumym(irc.IRCClient):
    ...
    def advance_turn(self):
        ...
        if not self.player_is_ai[self.current_turn]:
            ...
            ai = self.playing_ais[self.players[self.current_turn]]
            guess = ai.makeGuess()
            self.msg(self.chan, "la " + ai.name + " smadi le du'u zo " + guess + " valsi")
            self.guess(ai.name, guess)
